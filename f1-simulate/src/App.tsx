...
function Stats({ data }: { data: AppData }) {
  ...
  const maxPoints = remainingRaces.length * 25 + remainingSprints.length * 8;
    ...
    async function loadTallyAndWins() {
      ...
      try {
        ...
        if (tallyRes.ok) {
          ...
          for (const row of tallyJson.rows || []) {
            const num = parseInt(row["Driver Number"]) || 0;
            if (!num) continue;
            tallyMap[num] = {};
            for (const key of Object.keys(row)) {
              if (key !== "Driver Number" && key !== "Driver Name") {
                tallyMap[num][key] = Number(row[key] ?? 0);
              }
            }
          }
          if (!cancelled) setPositionTally(tallyMap);
        }
        if (winsRes.ok) {
          const winsJson = await winsRes.json();
          const winsMap: Record<number, { wins: number; podiums: number }> = {};
          for (const row of winsJson.rows || []) {
            const num = parseInt(row["Driver Number"]) || 0;
            if (!num) continue;
            winsMap[num] = {
        ...
        <div className="min-w-0 w-full">
          ...
          <div className="p-1 space-y-1">
            {current.map(([num, pts], idx) => {
              ...
              const driverWins = winsData?.[num]?.wins ?? 0;
              const driverPodiums = winsData?.[num]?.podiums ?? 0;
